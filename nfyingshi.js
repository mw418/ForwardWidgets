  ...
  $('.bt_img li').each(function () {
    ...
    if (title && postId) {
      items.push({
        id: 'nf:' + postId,
        type: 'url',
        title: title + (title.indexOf('季') !== -1 ? '' : ' 第' + extractSeasonInfo(title).seasonNumber + '季'),
        posterPath: poster,
        link: 'nf:' + postId,
        rating: rating,
...
function parseSearchCards($, siteUrl) {
  ...
  $("article.searart").each(function () {
    ...
    if (title && postId) {
      items.push({
        id: "nf:" + postId, type: "url", title: title + (title.indexOf('季') !== -1 ? '' : ' 第' + extractSeasonInfo(title).seasonNumber + '季'),
  ...
  try {
    ...
    $('a[href*="v_play"]').each(function () {
      ...
      if (epMatch) {
        ...
        episodeItems.push({
          id: epId, type: 'url', title: title + (title.indexOf('季') !== -1 ? '' : ' 第' + extractSeasonInfo(title).seasonNumber + '季'), coverUrl: poster, posterPath: poster, description: title + ' - ' + epTitle, season: si.seasonNumber, episode: epNum, playerType: 'system', link: epId,
    ...
    if (!episodeItems.length) {
      ...
      for (var ri = 0; ri < rawEps.length; ri++) {
        ...
        episodeItems.push({
          id: rId, type: 'url', title: title + (title.indexOf('季') !== -1 ? '' : ' 第' + extractSeasonInfo(title).seasonNumber + '季'), coverUrl: poster, posterPath: poster, description: title + ' - ' + re.title, season: rsi.seasonNumber, episode: reNum, playerType: 'system', link: rId,
    ...
    // Pre-resolve all videoUrls in parallel; childItems only for first episode
    var epTasks = [];
          ...
          if (info && info.urls.length > 0) {
            ...
            for (var q = 0; q < info.urls.length; q++) {
              ep.childItems.push({
                ...
                description: ep.title + ' - ' + (info.names[q] || ('画质' + (q + 1))),
              });
            }
          }
