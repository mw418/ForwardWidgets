  ...
  try {
    ...
    $('a[href*="v_play"]').each(function () {
      ...
      if (epMatch) {
        ...
        episodeItems.push({
          id: epId, type: 'url', title: title, coverUrl: poster, posterPath: poster, description: title + ' - ' + epTitle, season: si.seasonNumber, episode: epNum, playerType: 'system', link: epId,
    ...
    if (!episodeItems.length) {
      ...
      for (var ri = 0; ri < rawEps.length; ri++) {
        ...
        episodeItems.push({
          id: rId, type: 'url', title: title, coverUrl: poster, posterPath: poster, description: title + ' - ' + re.title, season: rsi.seasonNumber, episode: reNum, playerType: 'system', link: rId,
