...
function _xtime(a) { return (a & 0x80) ? ((a << 1) ^ 0x1b) & 0xff : a << 1; }

function _keyExpansion(key) {
  var i, j, t, rk = new Array(11 * 16);
...
function _addRoundKey(state, rk, round) {
  for (var i = 0; i < 16; i++) state[i] ^= rk[round * 16 + i];
}

function _subBytes(state) { for (var i = 0; i < 16; i++) state[i] = _SBOX[state[i]]; }
function _invSubBytes(state) { for (var i = 0; i < 16; i++) state[i] = _INV_SBOX[state[i]]; }

function _shiftRows(s) {
  var t;
  t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
  t = s[2]; s[2] = s[10]; var u = s[6]; s[6] = s[14]; s[14] = t; s[10] = u;
...
function _invShiftRows(s) {
  ...
  t = s[2]; s[2] = s[10]; var u = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = u;
  t = s[3]; s[3] = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = t;
}

function _mixColumn(col) {
  var a = col[0], b = col[1], c = col[2], d = col[3];
  var a2 = _xtime(a), a3 = a2 ^ a, b2 = _xtime(b), b3 = b2 ^ b, c2 = _xtime(c), c3 = c2 ^ c, d2 = _xtime(d), d3 = d2 ^ d;
  col[0] = a2 ^ b3 ^ c ^ d;
  col[1] = a ^ b2 ^ c3 ^ d;
  col[2] = a ^ b ^ c2 ^ d3;
  col[3] = a3 ^ b ^ c ^ d2;
...
function _invMixColumn(col) {
  var a = col[0], b = col[1], c = col[2], d = col[3];
  var a2 = _xtime(a), a4 = _xtime(a2), a8 = _xtime(a4);
  var b2 = _xtime(b), b4 = _xtime(b2), b8 = _xtime(b4);
  var c2 = _xtime(c), c4 = _xtime(c2), c8 = _xtime(c4);
  var d2 = _xtime(d), d4 = _xtime(d2), d8 = _xtime(d4);
  col[0] = a8 ^ a4 ^ a2 ^ b8 ^ b2 ^ b ^ c8 ^ c4 ^ c ^ d8 ^ d ^ d4 ^ d2;
  col[1] = a8 ^ a4 ^ a2 ^ a ^ b8 ^ b2 ^ b ^ c8 ^ c4 ^ c2 ^ d8 ^ d ^ d4;
  col[2] = a8 ^ a4 ^ a ^ a2 ^ b8 ^ b4 ^ b2 ^ c8 ^ c2 ^ c ^ d8 ^ d4 ^ d;
  col[3] = a8 ^ a2 ^ a ^ b8 ^ b4 ^ b2 ^ b ^ c8 ^ c4 ^ c2 ^ c ^ d8 ^ d2 ^ d;
}

function _mixColumns(state) {
  for (var i = 0; i < 4; i++) {
    var off = i * 4;
    var col = [state[off], state[off+1], state[off+2], state[off+3]];
    _mixColumn(col);
    state[off] = col[0]; state[off+1] = col[1]; state[off+2] = col[2]; state[off+3] = col[3];
  }
}

function _invMixColumns(state) {
  for (var i = 0; i < 4; i++) {
    var off = i * 4;
    var col = [state[off], state[off+1], state[off+2], state[off+3]];
    _invMixColumn(col);
...
function _aesDecryptBlock(block, rk) {
  var state = block.slice();
  _addRoundKey(state, rk, 10);
  for (var r = 9; r >= 1; r--) {
    _invShiftRows(state);
    _invSubBytes(state);
    _addRoundKey(state, rk, r);
...
function aesDecrypt(ciphertext) {
  var key = [], iv = [];
  for (var i = 0; i < 16; i++) { key[i] = AES_KEY.charCodeAt(i) & 0xff; iv[i] = AES_IV.charCodeAt(i) & 0xff; }
  var rk = _keyExpansion(key);
  var blocks = [];
  ...
  for (var b = 0; b < blocks.length; b++) {
    var dec = _aesDecryptBlock(blocks[b], rk);
...
function aesDecryptB64(base64str) {
  ...
  while (i < base64str.length) {
    ...
    var c = b64chars.indexOf(base64str.charAt(i++));
    var d = b64chars.indexOf(base64str.charAt(i++));
    bytes.push((a << 2) | (b >> 4));
